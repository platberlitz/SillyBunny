...
async function getClientVersion() {
    try {
        ...
        displayVersion = SILLYBUNNY_UI_VERSION;
...
function registerSillyBunnyServiceWorker() {
    ...
    const register = () => {
        navigator.serviceWorker.register('/sw.js?v=20260522f', { updateViaCache: 'none' }).then((registration) => {
...
jQuery(async function () {
    ...
    function syncRangeCounter($slider) {
        if (!($slider instanceof jQuery) || $slider.length === 0) {
            return;
        }
        ...
        $(`#${sliderId}_counter`).val(value);
        $(`.neo-range-input[data-for="${sliderId}"], .range-block-counter input[data-for="${sliderId}"]`).val(value);
    }
