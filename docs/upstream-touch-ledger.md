...
| Protecting tests | `tests/chat-scroll-edges.test.js`, `tests/mobile-streaming.test.js`, `tests/chat-render-lifecycle-anchor.test.js`, `tests/chat-send-scroll.e2e.js`, `tests/chat-scroll-regressions.e2e.js`, future lifecycle unit tests. |
| Validation | `npm run lint`, `npm run lint --prefix tests -- chat-send-scroll.e2e.js chat-scroll-regressions.e2e.js chat-scroll-regression-helpers.js chat-render-lifecycle-anchor.test.js`, `npm run test:unit --prefix tests -- chat-scroll-edges.test.js mobile-streaming.test.js chat-render-lifecycle-anchor.test.js`, `npm run check:frontend-budgets`, `SILLYBUNNY_TEST_BASE_URL=http://127.0.0.1:4567 npm run test:e2e --prefix tests -- chat-scroll-regressions.e2e.js chat-send-scroll.e2e.js`. |
