...
function normalizeSummarySource(source) {
    return source === summary_sources.extras ? summary_sources.main : source;
}
...
function loadSettings() {
    ...
    extension_settings.memory.source = normalizeSummarySource(extension_settings.memory.source);
...
function onSummarySourceChange(event) {
    const value = normalizeSummarySource(event.target.value);
    $('#summary_source').val(value);
