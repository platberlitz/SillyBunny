...
export async function getWorldEntry(name, data, entry, options = {}) {
    ...
    orderInput.css({ width: '100%', maxWidth: 'none' });
    ...
    headerTemplate.find('input[name="depth"]').css({ width: '100%', maxWidth: 'none' });
    ...
    function addEditorDrawerContent(targetOutlet = editOutlet, { desktop = false } = {}) {
        ...
        const agentBlacklistedInput = editTemplate.find('input[name="agentBlacklisted"]');
        agentBlacklistedInput.data('uid', entry.uid);
        agentBlacklistedInput.on('input', async function (_, options = {}) {
            const uid = $(this).data('uid');
            const value = $(this).prop('checked');
            data.entries[uid].agentBlacklisted = value;
            setWIOriginalDataValue(data, uid, 'extensions.agent_blacklisted', data.entries[uid].agentBlacklisted);
            if (!options.noSave) {
                await saveWorldInfo(name, data);
            }
        });
        agentBlacklistedInput.prop('checked', entry.agentBlacklisted ?? false).trigger('input', { noSave: true });
...
export const newWorldInfoEntryDefinition = {
    ...
    agentBlacklisted: { default: false, type: 'boolean' },
